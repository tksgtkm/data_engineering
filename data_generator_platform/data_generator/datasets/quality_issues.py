...
class DuplicatesRowDecorator(RowDecorator):
    ...
    def decorate(self, row_to_decorate: DataGeneratorEntity):
        should_be_duplidated = random.choices(population=[True, False], weights=[0.3, 0.6])[0]
        if should_be_duplidated:
...
class MissingFieldsRowDecorator(RowDecorator):
    ...
    def decorate(self, row_to_decorate: DataGeneratorEntity):
        ...
        all_callable_fields = ([key for key, value in row_to_decorate.__dict__.items() if not isinstance(value, callable_types)])
...
class LateRowDecorator(RowDecorator):
    ...
    def __init__(self, decorator_method: Callable[[int], DataGeneratorEntity], entity_index_from_decorator: int, max_late_rows: int = 100000):
    ...
    def decorate(self, row_to_decorate: DataGeneratorEntity) -> List[DataGeneratorEntity]:
        ...
        self.__late_rows.pus(row_to_decorate)

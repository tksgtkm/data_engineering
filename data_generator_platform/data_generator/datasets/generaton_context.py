...
class DatasetGenerationContext:
    ...
    def get_rows_to_generate_with_maybe_decorators(self) -> List[RowDecorator]:
        ...
        number_of_duplicates = DatasetGenerationContext.calculate_percentage(
            rows_to_generate,
            self.duplicates_pertentage
        )
        logging.info(f'...{number_of_duplicates} are duplicates...')
        decorators = [
            DuplicatesRowDecorator(self.entity_generator.generate_row, i)
            for i in range(0, number_of_duplicates)
        ]

        number_of_missing_fields = DatasetGenerationContext.calculate_percentage(
            rows_to_generate,
            self.missing_fields_percentage
        )
        logging.info(f'...{number_of_missing_fields} have missing fields...')
        decorators[len(decorators):] = [
            MissingFieldsRowDecorator(self.entity_generator.generate_row, i)
            for i in range(len(decorators), len(decorators) + number_of_missing_fields)
        ]

        number_of_unprocessable_rows = DatasetGenerationContext.calculate_percentage(
            rows_to_generate,
            self.unprocessable_rows_percentage
        )
        logging.info(f'...{number_of_unprocessable_rows} are unprocessable...')
        decorators[len(decorators):] = [
            UnprocessableRecordRowDecorator(self.entity_generator.generate_row, i)
            for i in range(len(decorators), len(decorators) + number_of_unprocessable_rows)
        ]

        number_of_rows_without_issues = max(
            0,
            int(rows_to_generate - number_of_duplicates - number_of_missing_fields - number_of_unprocessable_rows) 
        )
        logging.info(f'...{number_of_rows_without_issues} don''t have any issues...')
        decorators[len(decorators):] = [
            EmptyRowDecorator(self.entity_generator.generate_row, i)
            for i in range(len(decorators), len(decorators) + number_of_rows_without_issues)
        ]

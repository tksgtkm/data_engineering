...
from data_generator.datasets.generaton_context import EntityGenerator
...
class DeviceEntityGenerator(EntityGenerator):
    ...
    def generate_row(self, index: int) -> DataGeneratorEntity:
        logging.debug(f'Generating Devices for Index {index}')
        try:
            device_from_index = self.generate_row[index]
